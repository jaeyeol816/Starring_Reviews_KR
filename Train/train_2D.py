...
# For TensorBoard
writer = SummaryWriter('runs/exp_2/case_D')

# For saving
model_save_path = './models/Train_2D.pt'
...
# DataLoader 인스턴스화
train_movie_dataloader = torch.utils.data.DataLoader(data_movie_train, batch_size=batch_size, num_workers=5, shuffle=True)
train_place_dataloader = torch.utils.data.DataLoader(data_place_train, batch_size=batch_size, num_workers=5, shuffle=True)
...
for e in range(num_epochs-2):
    ...
    for batch_id, (token_ids, valid_length, segment_ids, label) in tqdm(enumerate(train_movie_dataloader), total=len(train_movie_dataloader)):
        ...
        if batch_id % log_interval == 0:
            print("movie epoch {} batch id {} loss {} train acc(MAE inverse) {}".format(e+1, batch_id+1, loss.data.cpu().numpy(), train_acc / (batch_id+1)))
	    			# for TensorBoard
            writer.add_scalar('training loss', loss.data.cpu().numpy(),  (e)*len(data_movie_train) + (batch_id+1)*batch_size)
            # writer.close()
            writer.add_scalar('training accuracy (MAE inverse)', train_acc / (batch_id+1), (e)*len(data_movie_train) + (batch_id+1)*batch_size)
            # writer.close()
    ...
    print("movie epoch {} test acc(MAE inverse) {}".format(e+1, test_acc / (batch_id+1)))
...
for e in range(num_epochs + 2 * int(len(data_movie_train) / len(data_place_train))):
	...
	for batch_id, (token_ids, valid_length, segment_ids, label) in tqdm(enumerate(train_place_dataloader), total=len(train_place_dataloader)):
			...
			if batch_id % log_interval == 0:
					...
					# for TensorBoard
					writer.add_scalar('training loss', loss.data.cpu().numpy(), n_iter_after_moive +  (e)*len(data_place_train) + (batch_id+1)*batch_size)
					# writer.close()
					writer.add_scalar('training accuracy (MAE inverse)', train_acc / (batch_id+1), n_iter_after_moive +  (e)*len(data_place_train) + (batch_id+1)*batch_size)
					# writer.close()
