...
# For TensorBoard
writer = SummaryWriter('runs/exp_1/case_A')

# For saving
model_save_path = './models/train_1A.pt'
...
for e in range(num_epochs):
    ...
    for batch_id, (token_ids, valid_length, segment_ids, label) in tqdm(enumerate(train_dataloader), total=len(train_dataloader)):
        ...
        if batch_id % log_interval == 0:
            print("epoch {} batch id {} loss {} train acc(MAE inverse) {}".format(e+1, batch_id+1, loss.data.cpu().numpy(), train_acc / (batch_id+1)))
	    			# for TensorBoard
            writer.add_scalar('training loss', loss.data.cpu().numpy(), (e+1)*len(data_train) + (batch_id+1)*batch_size)
            # writer.close()
            writer.add_scalar('training accuracy (MAE inverse)', train_acc / (batch_id+1), (e+1)*len(data_train) + (batch_id+1)*batch_size)
            # writer.close()
    ...
    writer.add_scalar('test accuracy (MAE inverse)', test_acc / (batch_id+1), e+1)
    # writer.close()
